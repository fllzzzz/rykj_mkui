<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="分享用户" prop="username">
        <el-input
      ...
      <el-form-item label="分享用户Id" prop="userId">
        <el-input
          v-model="queryParams.userId"
          placeholder="请输入分享用户Id"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="被分享用户" prop="shareUsername">
        <el-input
          v-model="queryParams.shareUsername"
          placeholder="请输入被分享用户"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="被分享用户Id" prop="sharedUserId">
        <el-input
          v-model="queryParams.sharedUserId"
          placeholder="请输入被分享用户Id"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="提取码" prop="accessCode">
        <el-input
    ...
    <el-table v-loading="loading" :data="shareList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="分享用户" align="center" prop="username" />
      <el-table-column label="分享用户Id" align="center" prop="userId" />
      <el-table-column label="被分享用户" align="center" prop="shareUsername" />
      <el-table-column label="被分享用户Id" align="center" prop="sharedUserId" />
      <el-table-column label="过期时间" align="center" prop="expiredTime" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.expiredTime, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="提取码" align="center" prop="accessCode" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="分享用户" prop="username">
          <el-input v-model="form.username" placeholder="请输入分享用户" />
        </el-form-item>
        <el-form-item label="分享用户Id" prop="userId">
          <el-input v-model="form.userId" placeholder="请输入分享用户Id" />
        </el-form-item>
        <el-form-item label="被分享用户" prop="shareUsername">
          <el-input v-model="form.shareUsername" placeholder="请输入被分享用户" />
        </el-form-item>
        <el-form-item label="被分享用户Id" prop="sharedUserId">
          <el-input v-model="form.sharedUserId" placeholder="请输入被分享用户Id" />
        </el-form-item>
        ...
        <el-form-item label="提取码" prop="accessCode">
          <el-input v-model="form.accessCode" placeholder="请输入提取码" />
        </el-form-item>
...
<script>
import { listShare, getShare, delShare, addShare, updateShare } from "@/api/system/share";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除文档分享编号为"' + ids + '"的数据项？').then(function() {
        return delShare(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/share/export', {
        ...this.queryParams
